...
# -----------------------------
# Mobile-friendly side-by-side buttons
# -----------------------------
# Add a tiny spacer column to prevent stacking in portrait mode
col1, col2, col3 = st.columns([1,0.05,1])

with col1:
    reset_pressed = st.button("Reset Shoe", key="reset_button")
...
if calculate_pressed:
    if not player_hand_input or not dealer_up:
        ...
        # Capture printed output from guidance function
        f = io.StringIO()
        with redirect_stdout(f):
            guidance_for_all_hands(shoe, dealer_up_rank, [player_hand], rules)
        output = f.getvalue()

        # Display output with best action highlighted in red
