...
raw_seen = st.text_input("Cards dealt so far (ranks only, e.g., '3 10 A Q')")
player_hand_input = st.text_input("Your hand (ranks only, e.g., '8 8')")
dealer_up = st.text_input("Dealer upcard (rank only, e.g., '6')")

if st.button("Calculate"):
    if not raw_seen or not player_hand_input or not dealer_up:
        st.warning("Please enter all fields!")
    else:
        seen_cards = parse_seen_cards(raw_seen)
        shoe = build_shoe(decks=8, seen_cards=seen_cards)
        player_hand = parse_seen_cards(player_hand_input)
        dealer_up = dealer_up.upper()
        ...
        with redirect_stdout(f):
            guidance_for_all_hands(shoe, dealer_up, [player_hand], rules)
        output = f.getvalue()
