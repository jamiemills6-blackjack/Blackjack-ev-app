...
def parse_cards(input_str):
    ...
    cards = [c.strip().upper() for c in input_str.split(",") if c.strip()]
    for c in cards:
        if c not in VALID_CARDS:
            raise ValueError(f"Invalid card entered: {c}. Must be one of {sorted(VALID_CARDS)}")
    return cards
...
# Header: single line, main text 10% smaller, (Mobile) same size
st.markdown(
...
if calc_btn:
    try:
        ...
        if dealer_card not in VALID_CARDS:
            st.error(f"Invalid dealer card: {dealer_card}. Must be one of {sorted(VALID_CARDS)}")
        else:
