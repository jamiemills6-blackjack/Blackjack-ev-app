...
def recommend_action(player_cards, dealer_up):
    # Simplified perfect basic strategy logic
...
# Session state init
for key in ["shoe", "seen_cards", "player", "dealer", "others"]:
    if key not in st.session_state:
        st.session_state[key] = build_shoe(8) if key=="shoe" else [] if key=="seen_cards" else ""
...
div.stButton > button:nth-child(2) {background-color: #d3d3d3; color: black;}
...
if next_btn:
    for key in ["player", "others", "dealer"]:
...
# -------------------
# JS for Enter key auto-focus
# -------------------
components.html("""
<script>
const inputs = window.parent.document.querySelectorAll('input[type="text"]');
if(inputs.length===3){
  inputs[0].addEventListener('keydown', e => {if(e.key==='Enter'){inputs[1].focus();}});
  inputs[1].addEventListener('keydown', e => {if(e.key==='Enter'){inputs[2].focus();}});
  inputs[2].addEventListener('keydown', e => {if(e.key==='Enter'){inputs[1].focus();}});
}
</script>
""", height=0)
